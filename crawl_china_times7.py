...
conn = get_conn()
cur = conn.cursor()
create_table="CREATE TABLE IF NOT EXISTS news_data2(day date, news_title varchar(512),link varchar(512),src varchar(15),content varchar(3096),max_img_url varchar(512))"
    ...
    if item.find("a") is None:# or max_img_urls[count]=='':
        ...
        for item2 in items2:
            if item2.findAll("p") is None:
                ...
                content2=str(content2).replace('[', '    ')
                content2=str(content2).replace(']', '')
                content2=content2.encode('UTF-8').decode()
                print('content2')
                print(content2)
                #hrefs.append(href)
                #print('href')
                ...
                if max_imgs[count]!='':
                    ...
                    sql='replace into news_data2 values(%s,%s,%s,%s,%s,%s)'
                    args=(str(datetime.today().strftime('%Y-%m-%d')),title,href,'中國時報',content2,max_img_urls[count])
